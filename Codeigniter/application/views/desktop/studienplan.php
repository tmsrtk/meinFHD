              	...
              	<?php echo '<div id="popWindow' . $data['KursID'] . '" class="modal hide" >'; ?>
                	...
                	<div class="modal-header">
                  	...
                  	<div class="modal-body">
                  		  <?php echo '<input id="hoeren_' . $data['KursID'] . '" name="hoeren_' . $data['KursID'] . '" type="hidden" value="'. $data['Teilnehmen'] .'" />'; ?>
    	              	  <?php echo '<input id="schreiben_' . $data['KursID'] . '" name="schreiben_' . $data['KursID'] . '" type="hidden" value="'. $data['Pruefen'] .'" />'; ?>
    	              	  <h4>
        	              	  <span>aktuelle Note: <?php echo $data['Notenpunkte']; ?></span>
            	          	  <button id="pruefen" class="id_<?php echo $data['KursID']; ?> btn btn-mini pull-right" data-toggle="button"><i class="icon-pencil"></i> Prüfen</button>
                	      	  <button id="teilnehmen" class="id_<?php echo $data['KursID']; ?> btn btn-mini pull-right" data-toggle="button"><i class="icon-headphones"></i> Teilnehmen</button>
                    	  </h4>
				          <br />
    	                  <h4>
                     	 ...
                     	 <h4>
        	                  ...
        	                  <?php for ( $n = 0 ; $n < count($semester); $n++ ) : ?>						
                                  <option value="<?php echo $n; ?>" selected="<?php echo ($n == $semesterNum) ? 'selected' : ''; ?>"><?php echo $n.'. Semester'; ?></option>
                              <?php endfor;?>               
                          </select>
                      </h4>
                  </div>
